...
let totalPrice = dataList.reduce((acc, product) => acc + product.price, 0)
console.log(`Загальна вартість - ${totalPrice}`)

// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).

let reducedPriceNum = dataList.reduce(
   (acc, product) => (product.price < product.old_price ? ++acc : acc),
   0
)
console.log(`Кількість товарів, у яких ціна зменшилась - ${reducedPriceNum}`)
...
for (const product of dataList) {
   if (product.sell_status === 'available')
      availableProductsList.push(product.id)
}
...
function getNewObject({ id, price, old_price, usd_price }) {
...
for (const product of dataList) {
   if (product.sell_status === 'available')
      newAvailableProductsList.push(getNewObject(product))
}

// let newAvailableProductsList = dataList
